module.exports = {
									...
									if(!_.isEmpty(bittrexExchange)){
										...
										bittrexTickers.exec(function(err,bittrexTickers){
											if(!_.isEmpty(bittrexTickers)){
												var bittrexTickers=JSON.parse(bittrexTickers.tickers).result;
												...
												ExchangeList.findOne({name:'coinmarketcap'},function(err, coinMarketCapExchange){
													if(!_.isEmpty(coinMarketCapExchange)){
														...
														coinMarketTickers.exec(function(err,coinMarketTickers){
															var coinMarketTickers=JSON.parse(coinMarketTickers.tickers);
																	...
																	bitfinexTickers.exec(function(err,bitfinexTickers){
																		if(!_.isEmpty(bitfinexTickers)){
																			...
																			if(_.isEmpty(isSocket)){
																				callBack({gdax:gdaxProducts, bittrex:bittrexTickers, coinmarket:coinMarketTickers,bitfinex:bitfinexTickers});
																			}else{
																				sails.sockets.blast('exchangeData',{gdax:gdaxProducts, bittrex:bittrexTickers, coinmarket:coinMarketTickers,bitfinex:bitfinexTickers});
																			}
																		}
																		else{
																			if(_.isEmpty(isSocket)){
																				callBack({gdax:gdaxProducts, bittrex:bittrexTickers, coinmarket:coinMarketTickers,bitfinex:[]});
																			}else{
																				sails.sockets.blast('exchangeData',{gdax:gdaxProducts, bittrex:bittrexTickers, coinmarket:coinMarketTickers,bitfinex:[]});
																			}
																		}
																	});
																}
														...
														if(_.isEmpty(isSocket)){
															callBack({gdax:gdaxProducts, bittrex:bittrexTickers, coinmarket:coinMarketTickers});
														}else{
															sails.sockets.blast('exchangeData',{gdax:gdaxProducts, bittrex:bittrexTickers, coinmarket:coinMarketTickers});
														}
													}
												});
											}
												...
												if(_.isEmpty(isSocket)){
												callBack({gdax:[], bittrex:[], coinmarket:[],bitfinex:[]});
												}else{
													sails.sockets.blast('exchangeData',{gdax:[], bittrex:[], coinmarket:[],bitfinex:[]});
												}
											}
										});
									}
	...
	volume_24_hour_currency:function(callBack){
		var _ = require('lodash');
		ExchangeList.find({currencies:{'!':null}},function(err, currencies){
	...
	volume_24_hour_exchange:function(callBack){
		var _=require('lodash');
		ExchangeList.find({products:{'!':null},name:{'!':'bitfinex'}},function(err, exchanges){
